...
//    }
    ...
    public List<BoardImage> saveImageOnS3ver2(Board board, List<MultipartFile> multipartFiles) throws IOException {
        List<BoardImage> images = new ArrayList<>();

        if (multipartFiles.isEmpty()) {
            return images;
        }
        String fileName = "board_images/" + board.getBoardId() + "board_";
        String thumbnailName = "board_thumbnail/board_of_" + board.getBoardId() + "/thumbnail_of_image_";
            ...
            if (!image.isEmpty()) {
                ...
                if (ObjectUtils.isEmpty(contentType)) {
                    ...
                    BoardImage boardImage = BoardImage.builder()
                            ...
                            .fileName(fileName)
                            .s3Url(amazonS3.getUrl(bucket, fileName).toString())
                            .thumbnailUrl(amazonS3.getUrl(bucket, thumbnailName).toString())
                            .fileSize(image.getSize())
                            .build();
                    ...
                    String absolutePath = new File("").getAbsolutePath() + "/";
                    // 저장 경로 설정
                    String path = "images/board_" + board.getBoardId();
                    File file = new File(path);
        ...
        board.setThumbnail("https://pre-032-bucket.s3.ap-northeast-2.amazonaws.com/" + thumbnailName + 0);
        boardRepository.save(board);
