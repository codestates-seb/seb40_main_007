...
export default function TrainStationCard() {
  ...
  return (
    <div className="lg:w-[1400px] pt-1 lg:mx-0 mx-5">
      <div className="w-full grid grid-cols-4 lg:gap-1 gap-[2px]">
        {trainStationInfo.map((el) => {
          return (
            <Link to={`/main/${el.id}`} key={el.id}>
              <div className="relative flex justify-center items-center text-white lg:text-3xl text-sm rounded-xl hover:ring hover:ring-[rgb(83,199,240)] hover:text-[rgb(83,199,240)] lg:hover:text-4xl">
                <img
